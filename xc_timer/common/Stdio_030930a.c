...

{
    ...
    distal_232_tx_pass_bit      = 0 ; // Default 0 - Passes TXD to distal   RS232 transmitter
    ...
    distal_422_used_bit         = 0 ; // Default 0 - Enables       distal   RS422 transmitter (active high)
  ...
  if    ( proximal_rs422_flag == 0 ) {  
    ...
    if  ( rs422_repeater_flag == 0 ) {        // Stand Alone - proximal RS232, no repeater,               single device
      ...
      if ( i_am_talking_to_pc_bit != 0 ) {
        distal_232_tx_pass_bit      = 0 ; 
        me_talk_to_pc_latch_bit     = 1 ; //                0x1e    
      endiff

      else {
        distal_232_tx_pass_bit      = 1 ; 
  ...
  endiff
    ...
    distal_232_tx_pass_bit      = 0 ;    // VVVV Get rid of this for all other cases
  #endif
...

{
  ...
  turn_on_not_cs_ram ( 0 ) ;                 // Turn on  External RAM
  turn_on_cs_ram2    ( 1 ) ;
  return_value = * xram_pointer ;
  turn_on_cs_ram2    ( 0 ) ;
  turn_on_not_cs_ram ( 1 ) ;                 // Turn off External RAM
