...
#define radio_sleep                  ( P3_2 )       // Pin 14  -  aka Int0
...
#define low_batt                     ( P1_3 )          // Pin  5  -  aka CEX0
  ...
  #define beeper(x)                  ( P1_4 =  (x) )   // Pin  6  -  aka CEX1
#endif
#if ( NEWEST_STUFF_FLAG != 0 ) 
  #define beeper(x)                  ( P1_4 = ~(x) )   // Pin  6  -  aka CEX1
#endif
...
//   read  data    byte      1     1
  ...
  #endif
  #if ( NEWEST_STUFF_FLAG != 0 ) 
    #define DEFAULT_COMMUNICATION_LATCH_BITS 0x1F
  #endif
...
//       (as long as interrupts are turned off while writing to iic)
    ...
    #define timer_interrupt_diagnostic_bit(x)       ( dummy_bit           = (x) ) 
    #define flicker0_bit                              dummy_bit
    #define flicker1_bit                              dummy_bit
    #define tp_trigger                                dummy_bit
    #define debounce_diagnostic_bit(x)              ( dummy_bit           = (x) )
    #define flash_diagnostic_bit(x)                 ( dummy_bit           = (x) )
    #define aux_serial_interrupt_diagnostic_bit(x)  ( dummy_bit           = (x) )
  #endif

  #if ( ( NEWEST_STUFF_FLAG == 0 ) && ( AUX_SERIAL_081902_STUFF_FLAG == 0 ) )                                                
  ...
  #if ( ( NEWEST_STUFF_FLAG == 0 ) && ( AUX_SERIAL_081902_STUFF_FLAG != 0 ) && ( NEW_AUX_SERIAL_082103_STUFF_FLAG == 0 ) )
    #define timer_interrupt_diagnostic_bit(x)       keep_power_on_bit = (x) ; CS_LATCH_PORT = communications_latch_byte ;
  ...
  #if  ( ( NEWEST_STUFF_FLAG == 0 ) && ( NEW_AUX_SERIAL_082103_STUFF_FLAG == 0 ) )
