...

{
      ...
      sprintf ( port_string, "\\\\.\\COM%d", port ) ;                  //   \\.\COM56
...

      {
        hComm = CreateFile (
                ...
                (LPCTSTR) port_string,       // LPCTSTR lpFileName - pointer to name of the file
          ...
          if ( hComm == INVALID_HANDLE_VALUE ) {
              sprintf ( error_string, "CreateFile failed  port: %s", port_string ) ;
              report_windows_error( __FILE__, __LINE__,  error_string ) ;
          }
